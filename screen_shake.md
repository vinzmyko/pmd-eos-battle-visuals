...
- Triggered by two moves — Earthquake (118) and Magnitude (296) — **plus one non-move source**: the value-5 "Earthquake" tileset stirring effect, played by the turn-limit wind warning. All three write the same `screen_shake_offset` field; the moves and the stirring effect use different drivers (see "Stirring-effect shake" below). No traps, explosions, abilities, or weather shake the screen.
...
## Stirring-effect shake (value 5)

The tileset `stirring_effect` field (see `tileset-properties.md`) value **5 ("Earthquake")** is a third writer of `screen_shake_offset` (`dungeon + 0x1A230`), separate from the two moves. It is played by the turn-limit **wind warning** (`DecrementWindCounter` → `FUN_022ec9a4` → `FUN_022ec8a8`), not by any move.

Unlike the move path, `FUN_022ec8a8` (NA `0x022EC8A8`) is a **self-contained player**: it busy-loops `AdvanceFrame`, writing `0x1A230` each iteration from its own curve table `DAT_022ec998` (stride `0xc0`, indexed by warning stage), toggling sign per inner frame and ramping the amplitude index down, then zeroes `0x1A230` on exit. It does **not** use `screen_shake_intensity` / `_reset` (`0x1A234` / `0x1A238`) or the per-frame consumer `FUN_022e34c8` — it drives the offset field directly. A music swap (`ChangeDungeonMusic`) brackets the sequence.

Net: same on-screen result and same output field as the moves, but a different code path and a different curve. The two systems share the *effect*, not the *driver*.
...
| `FUN_022ec8a8` | `0x022EC8A8` | Value-5 stirring screen-shake player (wind warning; writes `0x1A230` directly) |
| `FUN_022ec9a4` | `0x022EC9A4` | Reads tileset `stirring_effect`; dispatches value 5 → shake |
